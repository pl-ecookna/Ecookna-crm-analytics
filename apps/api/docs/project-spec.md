...
`Webhook -> S3 -> Postgres (direct) -> Deepgram -> OpenAI LLM -> Postgres (direct) -> BI`
...
1. Claim `new` записей (через RPC, fallback на select+optimistic update).
...
- `is_solved` вызывает `insight error` на текущей конфигурации
...
3. `disaproov_calls` живет в отдельном Postgres (`DB_DISAPPROVE_URL`), не в Supabase.
...
3. При пустом тексте в `callcenter` возможен fallback на `general` (как отдельная стратегия, пока не включена автоматически).
